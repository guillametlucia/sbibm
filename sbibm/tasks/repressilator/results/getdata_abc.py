...
algorithms = [rej_abc]#, smc_abc] 
names = ['rej_abc']#, 'smc_abc']
...
for i, algorithm in enumerate(algorithms): # Iterate over algorithms
    ...
    for b in budget:
        ...
        for i in range(4):#only 4 bcos didnt have time to generate more 
            start_time = time.process_time() # Start timing
            posterior_samples, _, _ = algorithm(task=task, num_samples=10_000, num_observation=i+1, num_simulations=b) #run algorithm and get posterior sampls
           ...
           # post_ready = np.hstack((posterior_samples_array, new_column)) #horizontal stack
            ...
            val_list.append(float(c2st_accuracy)) #append 10 accuracy values
...
for algorithm, simulations in c2st_values.items():
    ...
    for num_simulations, budget_dict in simulations.items():
        ...
        row = {'Algorithm': algorithm, 'Simulation Budget': num_simulations, 'Mean C2ST': mean_c2st, 'Standard Deviation C2ST': std_c2st, 'Mean Execution Time': mean_execution_time}
        # Append the row to the DataFrame
        c2st_stats = c2st_stats.append(row, ignore_index=True)


# Save DataFrame as CSV file
c2st_stats.to_csv('c2st_stats.csv', index=False)

#rom sbibm.visualisation import fig_metric
